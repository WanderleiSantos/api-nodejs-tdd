...
const TOKEN =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTAwMDAsIm5hbWUiOiJVc2VyICMxIiwibWFpbCI6InVzZXIxQG1haWwuY29tIn0.nFaQvIw5SdNCA-DgYu-LKKIDzIbEjn7h1DTMikYjQhU';
...
test('Deve inserir uma transferência com sucesso', () => {
  return request(app)
    ...
    .then(async res => {
      ...
      expect(res.body.description).toBe('Transfer #3');
...
describe('Ao tentar salvar uma transferência inválida...', () => {
  ...
  test('Não deve inserir se as contas pertencerem a outro usuário', () =>
    template({ acc_orig_id: 10002 }, 'Conta #10002 não pertence ao usuário'));
});
