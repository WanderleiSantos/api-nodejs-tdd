...
module.exports = app => {
  ...
  const validate = async transfer => {
    ...
    if (transfer.acc_orig_id === transfer.acc_dest_id)
    ...
    const accounts = await app
      .db('accounts')
      .whereIn('id', [transfer.acc_dest_id, transfer.acc_orig_id]);
    ...
    const transactions = [
      {
        ...
        type: 'O',
        acc_id: transfer.acc_orig_id,
        transfer_id: transferId,
        status: true,
      },
      {
        description: `Transfer from acc #${transfer.acc_orig_id}`,
        date: transfer.date,
